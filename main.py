...
def make_parser():
    ...
    repl = subparsers.add_parser(
        'interactive', description="Start an interactive command session "
                                   "to repeatedly run `interact` and `query`"
                                   "commands."
    )
    repl.add_argument(
...
class NEOShell(cmd.Cmd):
    ...
    def __init__(self, database, inspect_parser,
                 query_parser, aggressive=False, **kwargs):
    ...
    def precmd(self, line):
        ...
        if changed:
            files = ', '.join(
                str(f.relative_to(PROJECT_ROOT)) for f in changed
            )
            print(
