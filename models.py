...
class NearEarthObject:
    ...
    @property
    def _hazard_potential(self):
        """Return `is` if self.hazardous == True else `is not`."""
        if self.hazardous:
            return "is"
        else:
            return "is not"
    ...
    def __str__(self):
        ...
        return (f"A NearEarthObject {self.fullname} has a diameter of "
                f"{self.diameter:.3f} km and {self._hazard_potential} "
    ...
    def serialize(self):
        """Return the `NearEarthObject` as a dictionary."""
        return {
...
class CloseApproach:
    ...
    def time_str(self):
        """
        Return a formatted representation of `CloseApproach`'s approach time.
    ...
    def serialize(self):
        """Return the `CloseApproach` as a dictionary."""
        return {
